...
local url = http.request('https://www.api.beyond-dev.ir/getUser?token='..token_boss..'&user_id='..user_id)
local user = JSON.decode(url)
return user
end

function resolve_username(username)
local url = http.request('https://www.api.beyond-dev.ir/getUser?token='..token_boss..'&username='..username)
local user = JSON.decode(url)
return user
end
