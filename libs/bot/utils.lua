...
return "🔲┇ الـمـجـمـوعـه  لـيـسـت مـضـافـه ⚠️"
end
  ...
  if next(data[tostring(chat_id)]['banned']) == nil then --fix way
   ...
   message = '🔲┇قـائـمـه الاعـضـاء الـمـحـظـوريـن : \n'
...
return "🔲┇ الـمـجـمـوعـه  لـيـسـت مـضـافـه ⚠️"
  ...
  if next(data[tostring(chat_id)]['is_silent_users']) == nil then --fix way
   ...
   message = '🔲┇قـائـمـه الاعـضـاء الـمـكـتـومـيـن : \n'
...
function whitelist(chat_id)
    ...
    local i = 1
  if not data[tostring(chat_id)] then
return "🔲┇ الـمـجـمـوعـه  لـيـسـت مـضـافـه ⚠️"end
  ...
  if next(data[tostring(chat_id)]['whitelist']) == nil then --fix way
   return "🔲┇ عـذرا لا يـوجـد اعـضـاء  مـمـيـزوون  عـام فـي هـذه الـمـجـمـوعـه ⚠️"
end
   message = '🔲┇قـائـمـه الاعـضـاء الـمـمـيـزوون : \n'
 ...
 function gbanned_list(msg)
  ...
  if next(data['gban_users']) == nil then --fix way
   ...
   message = '🔲┇قـائـمـه الاعـضـاء الـمـحـظـوريـن عـام : \n'
...
return "🔲┇ الـمـجـمـوعـه  لـيـسـت مـضـافـه ⚠️"
  ...
  if next(data[tostring(msg.chat.id)]['filterlist']) == nil then --fix way
   return "🔲┇ عـذرا لا يـوجـد كـلـمـات مـمـنـوعـه  فـي هـذه الـمـجـمـوعـه ⚠️"
  end
  if not data[tostring(msg.chat.id)]['filterlist'] then
    data[tostring(msg.chat.id)]['filterlist'] = {}
    save_data(_config.moderation.data, data)
    end
   message = '🔲┇قـائـمـه الاعـضـاء الـكـلـمـات الـمـمـنـوعـه : \n'
 local i = 1
   for k,v in pairs(data[tostring(msg.chat.id)]['filterlist']) do
