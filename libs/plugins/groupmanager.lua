...
return  "🔲┇ لا يوجد ادمن في هذه المجموعه ☑️_"
end
...
.."`\n🗯 ¦ مطور الـسـورس : [TH3HELL](https://telegram.me/CHTH3CZAR)\n"
...
if matches[1] == "رفع عضو مميز"  and is_mod(msg) then
  ...
  data[tostring(msg.to.id)]['whitelist'][tostring(msg.reply.id)] = username
    save_data(_config.moderation.data, data)
    return "🔲┇ الـعـضـو  :  "..username.."\n🔲┇ الايـدي :   ["..msg.reply.id.."] \n🔲┇ تـم تـرقـيـتـه لـيـصـبـح عـضـو مـمـ��ـز  ☑️┇🔒"
      end
	  elseif matches[2] and matches[2]:match('^%d+') then
  if not getUser(matches[2]).result then
   return "🔲┇ لا يوجد عضو بهذا المعرف."
...
if matches[1] == "الترحيب"  and is_mod(msg) then
		...
		return "🔲┇  _مرحبا عزيزي_\n🔲┇ نورت المجموعه \n🔲┇ تابع : [TH3HELL](Telegram.Me/CHTH3CZAR)"
	end
end
...
🔲┇  مطور البوت : @CONTACT3BOT
🔲┇  للاستفسار راسلني : @CHTH3CZAR
]]
	sendPhoto(msg.to.id, "data/photos/TH3HELL.jpg", rsala, msg.id)
end
end
end
return {
